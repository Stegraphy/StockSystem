...
public class StockData {
    ...
    private static String time=sdf.format(new Date());
//    private static String time ="2019-07-0413-00-00";
...
//            <li data="DCFFITA10">10日排行<i></i></li>
                .map(l -> "http://nufm.dfcfw.com/EM_Finance2014NumericApplication/JS.aspx?type=ct&st=(BalFlowMain)&sr=-1&p=1&ps=50&js=var%20LCmTZqkg={pages:(pc),date:%222014-10-22%22,data:[(x)]}&"+l+"&cmd=C._AB&sty=DCFFITA")
                .map(l -> getData(l))
...
//        time = sdf.format(date);
        File f = new File("D:/Coding/stock_data/"+time+".txt");
        flag = false;
//        if(!f.exists()) {
...
//              .map(l -> "http://nufm.dfcfw.com/EM_Finance2014NumericApplication/JS.aspx?type=ct&st=(ChangePercent)&sr=-1&p="+num+"&ps=50&js=var%20CHKjvtxf={pages:(pc),date:%222014-10-22%22,data:[(x)]}&"+l+"&cmd=C._AB&sty=DCFFITA")
                .map(l -> "http://nufm.dfcfw.com/EM_Finance2014NumericApplication/JS.aspx?type=ct&st=(BalFlowMain)&sr=-1&p="+num+"&ps=50&js=var%20LCmTZqkg={pages:(pc),date:%222014-10-22%22,data:[(x)]}&"+l+"&cmd=C._AB&sty=DCFFITA")
               //       " http://nufm.dfcfw.com/EM_Finance2014NumericApplication/JS.aspx?type=ct&st=(BalFlowMain)&sr=   -1&p=       1&ps=50&js=var%20LCmTZqkg={pages:(pc),date:%222014-10-22%22,data:"}
                .map(l -> getData(l))
                .map(l -> l.substring(l.indexOf("[")+1,l.lastIndexOf("]")))
                .map(l -> l.replaceAll(" ", "").replaceAll("\t", ""))
                .flatMap(l -> Stream.of(l.split("\",\"")))
     ...
     */
        ...
        try {
            ...
            if (System.currentTimeMillis()%2==0) {
                ...
                connection.setRequestProperty("User-Agent","Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/75.0.3770.100 UBrowser/6.2.4094.1 Safari/537.36");
            }
